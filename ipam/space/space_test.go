...
func TestSpaceFree(t *testing.T) {
	...
	r, size := space.biggestFreeRange(entireRange)
	wt.AssertTrue(t, r.Start == ip(testAddr1) && size == 20, "Wrong space")
	...
	r, size = space.biggestFreeRange(entireRange)
	wt.AssertTrue(t, r.Start == ip("10.0.3.13") && size == 1, "Wrong space")

	// Free one at the end
	wt.AssertSuccess(t, space.Free(ip("10.0.3.23")))
	r, size = space.biggestFreeRange(entireRange)
	wt.AssertTrue(t, r.Start == ip("10.0.3.23") && size == 1, "Wrong space")
	...
	r, size = space.biggestFreeRange(entireRange)
	wt.AssertTrue(t, r.Start == ip("10.0.3.21") && size == 3, "Wrong space")
	...
	// Now get the biggest free space; should be 3.21
	r, size = space.biggestFreeRange(entireRange)
	wt.AssertTrue(t, r.Start == ip("10.0.3.10") && size == 4, "Wrong space")

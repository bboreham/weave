...
func ensureInterface(s *nl.NetlinkSocket, ifaceName string) (*net.Interface, error) {
	...
	netlinkDo(s, matchIfName(ifaceName))
...
func EnsureInterfaceAndMcastRoute(ifaceName string) (*net.Interface, error) {
	...
	netlinkDo(s, matchRoute(ifaceName, dest))
	return iface, err
}
...
func netlinkDo(s *nl.NetlinkSocket, f func(syscall.NetlinkMessage) (bool, error)) error {
	for {
		msgs, err := s.Receive()
		if err != nil {
			return err
